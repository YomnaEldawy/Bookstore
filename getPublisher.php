<?php
    ...
    $statement = mysqli_prepare($con, "SELECT b.Book_ISBN, title, publication_year, price, no_of_copies, threshold, Category_category_id, Publisher_publisher_id, pic_url, author_id, name FROM ((Book AS b INNER JOIN Book_has_Author AS a ) INNER JOIN Author) INNER JOIN Publisher  WHERE b.Book_ISBN = a.Book_ISBN and Author_author_id = author_id and and b.Publisher_publisher_id = publihser_id and  publisher_name = ?");
